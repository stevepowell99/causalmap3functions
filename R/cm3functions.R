...
convert_from_cm2 <- function(table_list,file_name) {
  ...
  links <-
    links %>%
    add_link_sources(statements,sources,questions = sess$file$questions)
  res <-
    list(
      files=files,
      links=links,
...
# statements <- sess$file$statements
# sources <- sess$file$sources
# questions <- sess$file$questions
add_link_sources <- function(links,statements,sources,questions){
  links %>%
    select(-any_of("source_id")) %>%
    left_join(statements %>% select(statement_id,source_id,question_id),by="statement_id") %>%
...
pipe_focus <- function(links,focus,any=F){
  ...
  if(any){
    links %>%
      mutate(found_from=map(.$from_label,~{any(str_detect(.,focus))}) %>% unlist) %>%
      mutate(found_to=map(.$to_label,~{any(str_detect(.,focus))}) %>% unlist) %>%
